...
if ($productCheck->num_rows == 0) {
    ...
    if ($updateType === 'quantity') {
        ...
        if (empty($newQuantity) && $newQuantity !== '0') {
            ...
            if ($success) {
                $adminResult = $db->getAdminByEmail($connection, $_SESSION['email']);
                $adminData = $adminResult->fetch_assoc();
                $adminName = $adminData['Name'] ?? $_SESSION['email'];
                
                $target = 'Product ' . $productId . ' Quantity';
                $db->insertHistory($connection, $_SESSION['email'], $adminName, 'Update', $target, $productData['product_quantity'], $newQuantity);
    ...
    elseif ($updateType === 'price') {
        ...
        if (empty($newPrice)) {
            ...
            if ($success) {
                ...
                $db->insertHistory($connection, $_SESSION['email'], $adminName, 'Update', $target, $productData['product_price'], $newPrice);
            ...
            if (in_array($fileType, $allowedTypes)) {
                ...
                if (move_uploaded_file($uploadFile['tmp_name'], $targetPath)) {
                    ...
                    if ($success) {
                        ...
                        $db->insertHistory($connection, $_SESSION['email'], $adminName, 'Update', $target, NULL, NULL);
    ...
    elseif ($updateType === 'delete') {
        ...
        if ($success) {
            ...
            $db->insertHistory($connection, $_SESSION['email'], $adminName, 'Delete', $target, NULL, NULL);
